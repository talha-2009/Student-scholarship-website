...
for (const htmlPath of htmlFilesFiltered) {
  ...
  // Fix Article mainEntityOfPage @id URLs (remove .html)
  html = html.replace(
    /("mainEntityOfPage"\s*:\s*\{[^}]*"@id"\s*:\s*"https:\/\/www\.opportunitynest\.org\/[^"]+)\.html(")/g,
    '$1/$2'
  );
  ...
  // 9f. Cookie consent banner (GDPR / EUUCP compliance for AdSense + GA4)
  if (!html.includes('id="on-consent-banner"')) {
