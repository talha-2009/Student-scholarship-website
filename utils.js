/**
 * Shared helpers for OpportunityNest Ã¢â‚¬â€ Supabase client, formatting, rendering, pagination.
 */
...
(function (ON) {
  ...
  ON.COUNTRY_FLAGS = {
    ...
    "Austria": "Ã°Å¸â€¡Â¦Ã°Å¸â€¡Â¹",
    "Canada": "Ã°Å¸â€¡Â¨Ã°Å¸â€¡Â¦",
    "Germany": "Ã°Å¸â€¡Â©Ã°Å¸â€¡Âª",
    "Switzerland": "Ã°Å¸â€¡Â¨Ã°Å¸â€¡Â­",
    "United Kingdom": "Ã°Å¸â€¡Â¬Ã°Å¸â€¡Â§",
    "United States": "Ã°Å¸â€¡ÂºÃ°Å¸â€¡Â¸",
    "Global": "Ã°Å¸Å’Â"


  ON.getCountryFlag = (country = "") => ON.COUNTRY_FLAGS[country] || "Ã°Å¸Å’Â";
  ...
  ON.renderMarkdown = (text = "") => {
    ...
    for (const block of blocks) {
      ...
      // Regular paragraph Ã¢â‚¬â€ may contain inline headings.
  ...
  // AdSense: Safe ad initialization Ã¢â‚¬â€ prevents duplicate pushes
  ON.pushAd = (selector) => {
    try {
    ...
    } catch (e) { /* AdSense may throw on empty slots Ã¢â‚¬â€ safe to ignore */ }
  ...
  ON.normalizeValue = (val) => {
    if (!val) return "";
    let clean = String(val).trim().toLowerCase();
    
    // Normalize plurals (except words ending in double 's' like business)
    if (clean.endsWith("s") && !clean.endsWith("ss")) {
      clean = clean.slice(0, -1);
    }
    
    // Map variations to base forms
    if (clean === "volunteering" || clean === "volunteer program" || clean === "volunteer work") {
      return "volunteer";
    }
    if (clean === "exchange program" || clean === "exchanges") {
      return "exchange";
    }
    if (clean === "leadership program" || clean === "youth leadership" || clean === "leader") {
      return "leadership";
    }
    if (clean === "conference program" || clean === "international conference") {
      return "conference";
    }
    if (clean === "it" || clean === "tech") {
      return "technology";
    }
  ...
  ON.matchCategory = (item, categoryVal) => {
    if (!categoryVal) return true;
    const normCategory = ON.normalizeValue(categoryVal);
    
    const normField = ON.normalizeValue(item.field);
    const normType = ON.normalizeValue(item.type);
    const normTitle = ON.normalizeValue(item.title);
    const normDesc = ON.normalizeValue(item.description);
    
    return normField.includes(normCategory) || 
           normType.includes(normCategory) || 
           normTitle.includes(normCategory) ||
           normDesc.includes(normCategory);
  ...
  ON.filterOpportunities = (items, filters = {}) => {
    ...
    const normType = type ? ON.normalizeValue(type) : "";
    const normCountry = country ? ON.normalizeValue(country) : "";
    const normFunding = funding ? ON.normalizeValue(funding) : "";
    const normLevel = level ? ON.normalizeValue(level) : "";
    ...
    return items.filter((item) => {
      ...
      // 2. Country match (normalized, supports US/USA and UK/United Kingdom mappings)
      let matchesCountry = true;
      if (normCountry) {
        const itemCountryNorm = ON.normalizeValue(item.country);
        matchesCountry = itemCountryNorm === normCountry ||
          (normCountry === "united state" && itemCountryNorm === "usa") ||
          (normCountry === "usa" && itemCountryNorm === "united state") ||
          (normCountry === "united kingdom" && itemCountryNorm === "uk") ||
          (normCountry === "uk" && itemCountryNorm === "united kingdom");
      }

      // 3. Type match
      const matchesType = !normType || 
        ON.normalizeValue(item.type).includes(normType) ||
        normType.includes(ON.normalizeValue(item.type));

      // 4. Category / Field match (uses category or field param)
      const catFilter = category || field;
      const matchesCategory = !catFilter || ON.matchCategory(item, catFilter);

      // 5. Funding match
      const matchesFunding = !normFunding || ON.normalizeValue(item.funding).includes(normFunding);

      // 6. Level match
      const matchesLevel = !normLevel || ON.normalizeValue(item.level).includes(normLevel);

      return matchesSearch && matchesCountry && matchesType && matchesCategory && matchesFunding && matchesLevel;
    });
    ...
    return `
      <article class="${cardClass}">
        ...
        <div class="card-actions">
          ...
          <a class="button button-primary" href="${ON.escapeHtml(applyHref)}"${applyTarget}>Apply Now <span aria-hidden="true">Ã¢â€ â€”</span></a>
        </div>
      </article>
    `;
  ...
  ON.SEO_OVERRIDES = [
    {
      ...
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ a fully funded research fellowship for scholars studying technology's social impact. Explore eligibility, funding, and deadlines.`,
    ...
    },
    {
      ...
      h1Suffix: "",
      metaDescription: (keyword, item) => `Join the ${keyword} Ã¢â‚¬â€ a global competition for master's teams with a CHF 25,000 prize pool and funded finals in Geneva. Check eligibility and apply.`,
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for a ${keyword} Ã¢â‚¬â€ paid global placements with UNICEF for students and recent graduates. Explore eligibility, stipend, and application steps.`,
      intro: (keyword, item) => `The ${keyword} Programme places talented students and recent graduates inside UNICEF, the United Nations agency dedicated to child rights, education, health, and protection worldwide. Open to undergraduate, master's, and PhD students Ã¢â‚¬â€ as well as graduates within the past two years Ã¢â‚¬â€ these placements are available across UNICEF offices and country programmes around the globe. Interns receive a monthly stipend and, subject to available funding, a one-time contribution toward travel and visa costs. Whether your background is in development, communications, public health, education, or data, this programme offers direct exposure to global humanitarian work and a meaningful start to an international career. Positions are posted on a rolling basis, so check the official careers portal regularly and apply early.`,
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ a fully funded UK master's scholarship for US students covering tuition, living costs, and airfare. Learn more and apply.`,
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ a fully funded German master's or PhD programme for development professionals. Explore eligible courses and deadlines.`,
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ McGill's flagship fully funded master's award with leadership development, mentorship, and a living stipend.`,
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ a fully funded residency at the University of Vienna for German-language writers, scholars, and translators. Explore eligibility and deadlines.`,
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ Cambridge University's flagship fully funded award for outstanding international postgraduates. Explore eligibility and deadlines.`,
      ...
      faqs: (keyword) => [
        { question: `What does the ${keyword} cover?`, answer: `Full university fees, a living stipend of approximately Ã‚Â£20,000 per year, round-trip airfare, and additional funding for academic development and family allowances.` },
        ...
        { question: `How competitive is this scholarship?`, answer: `Extremely competitive Ã¢â‚¬â€ around 40 scholarships are awarded annually from thousands of applications worldwide. Selection weighs academic merit, leadership, and commitment to social impact equally.` }
      ],
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ EU-funded fully funded master's programmes at top European universities. Explore eligible fields, scholarships, and deadlines.`,
      intro: (keyword, item) => `The ${keyword} (EMJMD) are prestigious, integrated, international study programmes jointly delivered by consortia of higher education institutions across Europe and beyond. Each programme lasts one to two years and awards a joint or multiple degree. The European Union funds full scholarships for outstanding students worldwide, covering participation costs, a monthly living allowance of Ã¢â€šÂ¬1,400, travel expenses, and insurance. With over 150 programmes spanning fields from renewable energy to human rights, public health to data science, EMJMD offers a unique opportunity to study in at least two different countries, gain intercultural competence, and build a pan-European professional network. If you are a bachelor's graduate seeking a truly international master's experience with full financial support, this is one of the most comprehensive funded programmes available.`,
      h2: (keyword) => `What Makes This EU Programme Unique`,
      faqs: (keyword) => [
        { question: `How much funding does the ${keyword} provide?`, answer: `Full scholarships cover tuition, a Ã¢â€šÂ¬1,400 monthly allowance, travel costs, and insurance for the entire programme duration (1Ã¢â‚¬â€œ2 years).` },
        { question: `Can I choose which countries to study in?`, answer: `Each EMJMD programme has a fixed mobility pattern Ã¢â‚¬â€ you study at a minimum of two different European universities as part of the consortium. You cannot pick individual institutions, but each programme publishes its exact mobility path.` },
    ...
    },
    {
      ...
      faqs: (keyword) => [
        ...
        { question: `Do I need to speak Japanese?`, answer: `Not initially. Many MEXT scholarships include a 1Ã¢â‚¬â€œ2 year Japanese language preparatory course before your degree begins. Some programmes at the graduate level are available in English.` }
      ],
      imageAlt: (keyword) => `${keyword} Japan study banner`
    },
    {
      id: "knight-hennessy",
      test: (title) => /Knight\s*[-Ã¢â‚¬â€œ]?\s*Hennessy/i.test(title),
      keyword: (item) => ON.resolveKeywordYear("Knight-Hennessy Scholars Program", item),
      titleDescriptor: "Stanford Award",
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ Stanford's flagship fully funded graduate scholarship with leadership development. Explore eligibility and how to apply.`,
      ...
      faqs: (keyword) => [
        ...
        { question: `Can I apply to any Stanford school?`, answer: `Yes Ã¢â‚¬â€ Knight-Hennessy Scholars can pursue any graduate degree across all seven Stanford schools: Business, Education, Engineering, Humanities & Sciences, Law, Medicine, and Earth Sustainability.` },
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ fully funded master's or career-development fellowship in peace and conflict studies at top universities worldwide.`,
    ...
    },
    {
      ...
      h1Suffix: "",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ a competitive paid internship at Google for computer science students. Explore eligibility, stipend, and application tips.`,
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ a fully funded master's scholarship at one of the world's top technical universities in Switzerland. Explore eligibility and benefits.`,
    ...
    },
    {
      ...
      h1Suffix: " (Fully Funded)",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ fully funded master's scholarships for students from developing countries at top universities worldwide.`,
    ...
    },
    {
      ...
      h1Suffix: "",
      metaDescription: (keyword, item) => `Apply for the ${keyword} Ã¢â‚¬â€ competitive research funding for explorers working in conservation, science, education, and storytelling. Explore eligibility and deadlines.`,
    ...
    },
    {
      ...
      faqs: (keyword) => [
        ...
        { question: `Do I need to work in a specific field?`, answer: `No Ã¢â‚¬â€ TED Fellows come from every field: science, art, technology, activism, journalism, medicine, engineering, and more. The common thread is bold ideas and measurable impact.` },
  ...
  ON.generateSEODescription = (item) => {
    ...
    const descVariants = [
      ...
      `${keyword}${typeFragment} Ã¢â‚¬â€ ${country}. ${funding}. Deadline ${deadline}. Find full eligibility, funding details, and application steps on OpportunityNest.`,
  ...
  ON.generateDetailH1 = (item) => {
    ...
    const h1SuffixVariants = [" (Fully Funded)", " Ã¢â‚¬â€ Full Guide & Application", " Ã¢â‚¬â€ Eligibility, Funding & How to Apply", " (Application Guide)", " Ã¢â‚¬â€ Details, Deadline & Requirements"];
  ...
  ON.generateDetailFAQs = (item) => {
    ...
    const eligibilityAnswers = [
      ...
      `The ${keyword}${typeFragment} accepts applications from ${level} candidates working in ${field}. Nationality requirements vary Ã¢â‚¬â€ some programmes are open globally, while others target specific regions or countries. Read the full criteria on the official website to confirm you qualify.`
    ];
    ...
    const fundingAnswers = isFullyFunded ? [
      ...
      `The ${keyword}${typeFragment} provides ${funding}. The specific financial support depends on the programme structure Ã¢â‚¬â€ some cover tuition only, while others include living allowances, travel grants, or research funding. Check the official listing for the full details of what is included.`,
      ...
      `The ${keyword}${typeFragment} offers ${funding}. What this covers varies Ã¢â‚¬â€ it may include partial tuition, a monthly stipend, or a one-time grant. Confirm the details on the official programme page before applying.`
    ];
    const fundingAnswer = ON.pickVariant(fundingAnswers, { title: (item.title || "") + "_faq2" });

    const applyAnswers = [
      `You can apply through the official programme website. The deadline is ${deadline}. Before applying, confirm your eligibility, prepare your motivation letter and CV, and arrange your references early. Most successful applicants start preparing at least 6Ã¢â‚¬â€œ8 weeks before the deadline.`,
      `Applications are submitted via the official programme portal. The deadline is ${deadline}. Start by checking your eligibility, then gather your documents Ã¢â‚¬â€ transcripts, references, and motivation letter Ã¢â‚¬â€ well in advance. Early preparation gives you time to refine each component.`,
    ...
    const competitivenessAnswers = [
      ...
      `Competition for the ${keyword}${typeFragment} is significant Ã¢â‚¬â€ it draws applications from ${country} and beyond. What separates successful candidates is specificity: a motivation letter that names the programme's unique features, connects them to your background, and shows a realistic plan for what comes next.`,
    ...
    const afterAnswers = [
      ...
      `Once selected, you will receive a formal acceptance with details about start dates, pre-arrival requirements, and any documentation you need to prepare Ã¢â‚¬â€ such as visa applications, health checks, or housing forms. The programme coordinators will walk you through each step.`,
    ...
    // Select 4 most useful FAQs Ã¢â‚¬â€ vary which one is dropped based on the item
    const allFaqs = [
  ...
  ON.generateWhoShouldApply = (item) => {
    ...
    const levelVariants = {
      ...
      master: [
        ...
        `Recent graduates with a passion for ${field} are the primary audience Ã¢â‚¬â€ this ${type} gives you the academic foundation and credentials to advance.`
      ],
      undergrad: [
      ...
      general: [
        ...
        `The programme is open to a broad range of applicants Ã¢â‚¬â€ what matters most is a clear motivation and a realistic plan for how you will use the experience.`
      ]
    ...
    const fundingVariants = [
      `Because this programme is fully funded, it is especially valuable for candidates who need financial support to study or work in ${country} Ã¢â‚¬â€ including those from low- and middle-income backgrounds.`,
      `Full funding means you can focus entirely on the programme without worrying about tuition, living costs, or travel Ã¢â‚¬â€ a significant advantage for international participants.`,
      ...
      `Fully funded programmes like this one are rare and competitive Ã¢â‚¬â€ if you qualify, the financial support allows you to commit fully to your studies or research in ${country}.`
    ];
    ...
    const intlVariants = [
      ...
      `${country} is an increasingly popular destination for international students and professionals Ã¢â‚¬â€ this programme makes the process easier with dedicated support structures.`
    ];
  ...
  ON.generateFundingExplained = (item) => {
    ...
    if (/fully\s*fund/i.test(funding)) {
      const variants = [
        `This ${type} covers the full cost of participation. For most recipients, this means tuition fees, a living stipend, health insurance, and travel costs are all included. The exact breakdown varies by programme Ã¢â‚¬â€ some cover accommodation directly, while others provide a monthly allowance that you manage yourself. Check the official programme page for the specific funding components, as they differ between recipients and may change from year to year.`,
    ...
    if (/partial/i.test(funding)) {
      const variants = [
        ...
        `Partial funding means the programme contributes to Ã¢â‚¬â€ but does not fully cover Ã¢â‚¬â€ your expenses. Common structures include a tuition discount, a fixed-term stipend, or a one-off relocation grant. Plan ahead by identifying other funding sources to bridge the gap.`,
        ...
        `This programme offers a meaningful but incomplete funding package. You can expect support for certain costs Ã¢â‚¬â€ often tuition or a monthly stipend Ã¢â‚¬â€ but should budget for expenses that fall outside the award. Many recipients successfully combine this with university bursaries, government loans, or crowdfunding.`,
        `Partial funding provides a financial foundation but requires you to plan for the rest. The programme typically covers one or two major cost categories Ã¢â‚¬â€ check the official listing to understand exactly which expenses are included and which you will need to fund independently.`
      ];
      return ON.pickVariant(variants, item);
    }
    if (/unpaid|volunteer/i.test(funding)) {
      const variants = [
        `This ${type} does not include a salary or stipend. However, unpaid positions at international organisations often provide other benefits: professional networking, mentorship, hands-on experience, and a strong credential for your CV. Some host organisations may offer travel reimbursements or help with visa costs Ã¢â‚¬â€ check the official listing for details.`,
    ...
    const fallbackVariants = [
      ...
      `This ${type} offers financial support, but the specifics depend on the programme year and your circumstances. The official website is the authoritative source for current funding details Ã¢â‚¬â€ review it carefully before applying.`
    ];
  ...
  ON.generateSelectionProcess = (item) => {
    ...
    const processSets = {
      ...
      master: [
        ["Academic transcript review Ã¢â‚¬â€ your grades, institution, and degree classification are assessed", "Motivation letter or statement of purpose explaining your goals and fit with the programme", "At least one reference from a professor or academic supervisor", "Interview (in person or online) to discuss your background and ambitions"],
        ["Evaluation of your academic record and degree relevance", "Review of your personal statement and any essays or writing samples", "Reference checks with your academic referees", "Selection interview Ã¢â‚¬â€ some programmes include a group activity or presentation"],
      ...
      internship: [
        ["CV and cover letter screening Ã¢â‚¬â€ recruiters look for relevant coursework, projects, and motivation", "Competency-based interview (often two rounds) focusing on problem-solving and teamwork", "Possible written test or case study relevant to the role"],
      ...
      competition: [
        ...
        ["Submission of your entry according to the competition brief and format requirements", "Expert review by a judging panel that scores entries against published criteria", "Shortlist announcement and final round Ã¢â‚¬â€ often involving a live presentation or demo"],
      ...
      general: [
        ["Application form and supporting documents review Ã¢â‚¬â€ your CV, transcripts, and motivation letter are assessed", "Motivation letter assessment focusing on your goals, fit, and what you bring to the programme", "Interview with the selection panel to discuss your application in more depth"],
        ["Initial eligibility check and document verification", "Qualitative review of your personal statement and references", "Selection interview Ã¢â‚¬â€ in person or virtual Ã¢â‚¬â€ to assess your motivation and suitability"],
    ...
    const outroVariants = [
      ...
      `Each stage is designed to filter for commitment and fit. Treat every component Ã¢â‚¬â€ from the application form to the interview Ã¢â‚¬â€ as an opportunity to demonstrate your seriousness.`,
      ...
      <section class="detail-section" aria-labelledby="explore-heading">
        ...
        <div class="explore-links">
          <div class="explore-group">
            ...
            <ul>
              ${countryLink ? `<li>${countryLink} Ã¢â‚¬â€ all programmes in ${ON.escapeHtml(country)}</li>` : ""}
          ...
          <div class="explore-group">
            ...
            <ul>
              <li><a href="/scholarships/united-kingdom/">Scholarships in the UK</a> Ã¢â‚¬â€ Chevening, Gates Cambridge, Rhodes &amp; more</li>
              <li><a href="/scholarships/germany/">Scholarships in Germany</a> Ã¢â‚¬â€ DAAD, Deutschlandstipendium &amp; more</li>
              <li><a href="/internships.html">International internships</a> Ã¢â‚¬â€ UN, CERN, Google &amp; more</li>
            </ul>
          </div>
        </div>
      </section>
  ...
  ON.renderDetailContent = (item, urgencyClass, categoryPage, categoryType) => {
    ...
    const docs = [];
    docs.push("<li><strong>Valid passport or national ID</strong> Ã¢â‚¬â€ must remain valid for the full programme duration</li>");
    docs.push("<li><strong>Curriculum Vitae (CV)</strong> Ã¢â‚¬â€ highlight academic, professional, and volunteer experience relevant to this programme</li>");
    docs.push("<li><strong>Academic transcripts</strong> Ã¢â‚¬â€ from all post-secondary institutions attended</li>");
    docs.push("<li><strong>Motivation letter or statement of purpose</strong> Ã¢â‚¬â€ explain why this specific programme, how it aligns with your goals, and what you bring</li>");
    if (level.match(/PhD|Doctoral|Postdoc|Research|Master/i) || typeLabel === "fellowship") {
      docs.push("<li><strong>Research proposal</strong> Ã¢â‚¬â€ required for research-focused programmes; outline your topic, methodology, and expected outcomes</li>");
    }
    docs.push("<li><strong>Letters of recommendation</strong> Ã¢â‚¬â€ typically two or three; give your recommenders at least 3Ã¢â‚¬â€œ4 weeks' notice</li>");
    if (country !== "Global" && country !== "Not specified") {
      docs.push("<li><strong>Proof of language proficiency</strong> Ã¢â‚¬â€ TOEFL, IELTS, or equivalent if the programme is not in your native language</li>");
    }
    if (typeLabel === "internship") {
      docs.push("<li><strong>Portfolio or work samples</strong> Ã¢â‚¬â€ if applicable to your field (design, writing, engineering, etc.)</li>");
    }
    const docsHtml = docs.join("");

    // Dynamic mistakes based on type
    const mistakes = [];
    mistakes.push("<li><strong>Submitting too close to the deadline</strong> Ã¢â‚¬â€ technical issues and time-zone differences cause last-minute failures. Aim to submit at least 48 hours early.</li>");
    if (typeLabel === "scholarship" || typeLabel === "fellowship") {
      mistakes.push("<li><strong>Writing a generic motivation letter</strong> Ã¢â‚¬â€ selection committees read hundreds of applications. Mention the specific programme by name, reference its unique features, and connect them to your background.</li>");
      mistakes.push("<li><strong>Not addressing the selection criteria</strong> Ã¢â‚¬â€ most programmes list what they evaluate. Structure your letter and CV to address each criterion directly.</li>");
    }
    if (typeLabel === "internship") {
      mistakes.push("<li><strong>Applying without researching the organisation</strong> Ã¢â‚¬â€ interviewers expect you to understand their mission, recent projects, and how your skills contribute.</li>");
    }
    mistakes.push("<li><strong>Ignoring eligibility requirements</strong> Ã¢â‚¬â€ nationality, age, degree level, and field restrictions are non-negotiable. Verify before investing time.</li>");
    mistakes.push("<li><strong>Asking for references at the last minute</strong> Ã¢â‚¬â€ give recommenders 3Ã¢â‚¬â€œ4 weeks and share your CV and motivation letter so they write specific, strong references.</li>");
    ...
    return `
      ...
      <div class="detail-header">
        ...
        <div class="hero-actions">
          <a class="button button-primary" href="${ON.escapeHtml(item.link || item.official_url)}" target="_blank" rel="noopener noreferrer">Apply Now <span aria-hidden="true">Ã¢â€ â€”</span></a>
      ...
      <section class="detail-section" aria-labelledby="timeline-heading">
        ...
        <p>${timelineIntro}</p>
        <ul>
          <li><strong>8Ã¢â‚¬â€œ10 weeks before:</strong> Read the full eligibility criteria and programme description. Confirm you qualify before investing time.</li>
          <li><strong>6Ã¢â‚¬â€œ8 weeks before:</strong> Contact your referees and share your CV and draft motivation letter with them.</li>
          <li><strong>4Ã¢â‚¬â€œ6 weeks before:</strong> Write your motivation letter, research proposal, or other essays. Get feedback from mentors or peers.</li>
          <li><strong>2Ã¢â‚¬â€œ3 weeks before:</strong> Gather transcripts, certificates, language test scores, and any other required documents.</li>
          <li><strong>1 week before:</strong> Complete the online application form. Review every field. Upload all documents.</li>
          <li><strong>48 hours before:</strong> Submit your application. Do not wait until the final day Ã¢â‚¬â€ technical issues and time-zone differences cause missed deadlines every year.</li>
        </ul>
      </section>
      ...
      <section class="detail-section" aria-labelledby="apply-heading">
        ...
        <p>${applyIntro}</p>
        <a class="button button-primary" href="${ON.escapeHtml(item.link || item.official_url)}" target="_blank" rel="noopener noreferrer">Visit Official Website <span aria-hidden="true">Ã¢â€ â€”</span></a>
      </section>
      ...
      ${item.created_at ? `<p class="microcopy">Listing added: ${new Date(item.created_at).toLocaleDateString('en-GB', {day:'numeric',month:'short',year:'numeric'})}. Details may change Ã¢â‚¬â€ confirm on the official website.</p>` : ''}
  ...
  ON.renderRelatedOpportunities = async (item) => {
    ...
    try {
      ...
      // Step 2: Fallback Ã¢â‚¬â€ same country, any type
      ...
      // Step 3: Fallback Ã¢â‚¬â€ same type, any country
      ...
      // Step 4: Fallback Ã¢â‚¬â€ any 4 recent opportunities
      if (related.length === 0) {
  ...
  // â”€â”€â”€ Filter Chips Component â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
